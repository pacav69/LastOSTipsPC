...
title LastUSB Installer [STARTING]
cd /d %~dp0
Set "Path=%Path%;%CD%;%CD%\bin;"
set maindir=%CD%
set localver=1001
set LastUSBver=23.09
set installertext=LastUSB Installer 
set format=Yes
set formatcolor=2F
...
title LastUSB Installer [UNSUPPORTED]
ver
...
cls
goto startup
powershell -c "$data = curl https://api.github.com/repos/LastOS/LastUSB_installer/git/refs/tag -UseBasicParsing | ConvertFrom-Json; $data[-1].ref -replace 'refs/tags/', '' | Out-File -Encoding 'UTF8' -FilePath './curver.ini'"
...
curl "https://raw.githubusercontent.com/LastOS/LastUSB_installer/main/update.bat" -o ./update.bat -s -L
...
echo.II-----------------------------------------------------------II
echo.IIII                                                       IIII
echo.IIII           LastUSB CONTAINS TOOLS THAT MAY             IIII
...
echo.II-----------------------------------------------------------II
echo.II-----------------------------------------------------------II[0m
echo.Please wait for a second, Read the above.  
PING localhost -n 2 >NUL
echo.                          Press any key to accept this warning.&& pause >nul


:startbinfiles
title LastUSB Installer [FILECHECK]
cls
goto start
...
rem pause
:menu
title LastUSB Installer [%localver%]
mode con:cols=100 lines=30
cls
rem powershell bin\licence.ps1 %lang%
echo.
rem pause
:menu2
cls
...
call Button 1 2 F2 "INSTALL LastUSB" 23 2 %formatcolor% "TOGGLE DRIVE FORMAT (CURRENTLY %format%)" 23 7 F9 "VISIT SITE" X _Var_Box _Var_Hover
echo.
echo.
echo.
echo.VERSION %localver% BY LastOS. 
echo.
rem powershell bin\motd.ps1 %lang%
...
If /I "%Errorlevel%"=="4" (
	cls & goto LastUSBsite
)
If /I "%Errorlevel%"=="5" (
	rem cls & goto recheck
)
...
:install2
title LastUSB Installer [CHOOSEINSTALL]
mode con:cols=100 lines=15
echo.We now need to find out what drive you will be installing to.
REM - FOLDER PROMPT STARTS
for /f "delims=" %%A in ('folderbrowse.exe "Please select the drive you want to install LastUSB on"') do set "folder=%%A"
REM - AND ENDS
...
cd %maindir% 
format %drivepath%: /FS:NTFS /X /Q /V:LastUSB /Y
goto installver
...
:installver
title LastUSB Installer [INSTALL!]
if exist "%CD%\LastUSB.USB.v%LastUSBver%.7z" (goto install4) else (goto installversion2)
:installversion2
if exist "%CD%\LastUSB.USB.v%LastUSBver%.zip.001" (goto hasher)
cls
mode con:cols=64 lines=18
echo.II-----------------------------------------------------------II
echo.II-----------------------------------------------------------II
echo.IIII                                                       IIII
echo.IIII         COULD NOT FIND THE LastUSB FILE(S).           IIII
...
echo.IIII                                                       IIII
echo.IIII                                                       IIII
echo.IIII        THE INSTALLER COULD NOT FIND LastUSB           IIII
echo.IIII                                                       IIII
...
set file="LastUSB.USB.v23.09.7z"
...
:install5
mode con:cols=100 lines=20
set file="LastUSB.USB.v%LastUSBver%.zip.001"
7z x -O%drivepath%: %file% -r -aoa
goto finishup
...
:finishup2
rem curl "https://raw.githubusercontent.com/LastOS/LastUSB_installer/main/icon.ico" -o %drivepath%:/autorun.ico -s -L
rem curl "https://raw.githubusercontent.com/LastOS/LastUSB_installer/main/hasher/CheckFiles.bat" -o %drivepath%:/CheckFiles.bat -s -L
rem cd /d %drivepath%:
rem start cmd /k CheckFiles.bat
CALL :RESOLVE "%~dp0..\.." PARENT_ROOT

set usbdrive=%drivepath%:
...
if EXIST "%usbdrive%\sources\boot.wim" mklink "%usbdrive%\SavePE_Install_LastOS.wim" "%usbdrive%\sources\boot.wim"
if EXIST "%usbdrive%\sources\bootmin.wim" mklink "%usbdrive%\Win10_PE.wim" "%usbdrive%\sources\bootmin.wim"
move /Y "%usbdrive%\bootmgr.efi" "%usbdrive%\efi"
goto _skip
...
:hasher
curl "https://raw.githubusercontent.com/LastOS/LastUSB_installer/main/hasher/drivefiles.md5" -o ./drivefiles.md5 -s -L
start QuickSFV.EXE drivefiles.md5
DEL drivefiles.md5 /Q
goto install5

:LastUSBsite
start https://www.lastos.org
goto menu2
...
for /f "delims=" %%A in ('folderbrowse.exe "Please select the drive you want to install LastUSB on"') do set "folder=%%A"
set drivepath=%folder:~0,1%
curl "https://raw.githubusercontent.com/LastOS/LastUSB_installer/main/hasher/CheckFiles.bat" -o %drivepath%:/CheckFiles.bat -s -L
cd /d %drivepath%:
...
::drivedown
::cls
::curl "https://raw.githubusercontent.com/LastOS/LastUSB_installer/main/download/drive.bat" -o ./drive.bat -s -L
::call drive.bat
::del drive.bat /Q
::goto installver
...
:tordown
cls
curl "https://raw.githubusercontent.com/LastOS/LastUSB_installer/main/download/tor.bat" -o ./tor.bat -s -L
call tor.bat
del tor.bat /Q
goto installver
...
:cdndown
cls
curl "https://raw.githubusercontent.com/LastOS/LastUSB_installer/main/download/cdn.bat" -o ./cdn.bat -s -L
call cdn.bat
del cdn.bat /Q
goto installver
